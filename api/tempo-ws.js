...
var $ws = {
    ...
    init: function (config) {
        ...
        require('./cbchat-log').debug("Init cbchat-ws module", 2);
        if (!$ws.config) {
    ...
    __endpointCallback: function (config) {
        ...
        return function (req, res) {
            ...
            require("./cbchat-log").debug("Endpoint '" + path + "' called", 1);
            if (config.body) {
                var code = (config.code) ? config.code : 200;
                var header = (config.header) ? config.header : {"Content-Type": "text/html"};
                res.writeHead(code, header);
                res.end(config.body);
                require("./cbchat-log").info("Endpoint '" + path + "' answered static document [" + code + "]");
            }
            else {
                res.writeHead(404, {"Content-Type": "text/html"});
                res.end("<html><head></head><body><h1>Not Found</h1></body></html>");
                require("./cbchat-log").warn("Endpoint " + req.method + " '" + path + "' not found");
            }
        }
    },
    start: function (callback) {
        require("./cbchat-log").debug("Start web server on port " + $ws.config.port, 2);
        try {
            $ws.server.listen($ws.config.port);
        }
        catch (e) {
            require("./cbchat-log").error('web server can\'t start because ' + e.message + ' [' + e.code + ']');
        }
    ...
    stop: function () {
        require("./cbchat-log").debug("Stop web server ", 2);
        return this;
    },
    ...
    response: function (res, type, message, data) {
        var obj = {
            ...
            send: function () {
                ...
                require("./cbchat-log").debug("okResponse sended to client");
            },
    ...
    dynamicRequestHandlerTest: function (config) {
        ...
        return function (req, res) {
            res.writeHead(200);
            res.end("this text was generated with require('./cbchat-ws').dynamicRequestHandlerTest. Le param_sample a été défini à " + config.param_sample);
            require("./cbchat-log").debug("Endpoint '" + config.path + "' answered with dynamic document", 3);
            return this;
        };
    }
};
