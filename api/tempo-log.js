...
var $log = {
     ...
     * @returns {log}
     */
    init: function (config, isDebug, geoll) {
        if (geoll) {
            this.setGeoll(geoll);
        }
        ...
        this.debug("Init cbchat-log module", 2);
        if (this.config.sqs) {
            this.sqs.init();
        }
    ...
    sqs: {
        ...
        init: function () {
            ...
            if (!require('./cbchat-resource').exist(conf.resource)) {
                throw new Error("resource '"
                        + conf.resource
                        + "' in log config 'sqs' doesn't exist");
            }
            this.resource = require('./cbchat-resource').get(conf.resource);
            this.isActive = true;
            return this;
        },
        /**
    ...
    couchbase: {
        ...
        init: function () {
            ...
            $log.couchbase.resource = require('./cbchat-resource').get(conf.resource);
